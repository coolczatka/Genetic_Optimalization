...
class RealRepresentationParameters(FunctionParametersInputs):
    @staticmethod
    def signalMapping():
        old = FunctionParametersInputs.signalMapping()
        old['CK'] = {'BRAK': 0, 'ARYTMETYCZNE': 1, 'HEURYSTYCZNE': 2}
        old['MK'] = {'BRAK': 0, 'RÓWNOMIERNA': 1}
        return old
    ...
    def __init__(self):
        CK_map = {v: k for k, v in RealRepresentationParameters.signalMapping()['CK'].items()}
        MK_map = {v: k for k, v in RealRepresentationParameters.signalMapping()['MK'].items()}
        KIND_map = {v: k for k, v in RealRepresentationParameters.signalMapping()['KIND'].items()}
        SELECTION_map = {v: k for k, v in RealRepresentationParameters.signalMapping()['SELECTION'].items()}
        self.setSgClass(sgclass=None)
        definition = [
            [
                sg.Column([
                ...
                sg.Column([
                    ...
                    [sg.Text('Ilość generacji'), sg.Input(key='_GENERATIONS_', enable_events=True, default_text=str(GC.config.generations), size=(20,1))],
                    [sg.Text('Wielkość populacji'), sg.Input(key='_POPULATIONSIZE_', enable_events=True, default_text=str(GC.config.populationSize), size=(20,1))],
                    [sg.Text('Rodzaj'), sg.Combo(list(RealRepresentationParameters.signalMapping()['KIND'].keys()), default_value=KIND_map[GC.config.kind], key='_KIND_', enable_events=True, size=(20,1))],
                    [sg.Text('Zakres'), sg.Input(key='_RANGE_', enable_events=True, default_text=str(str(GC.config.range[0])+','+str(GC.config.range[1])), size=(20,1))],
                    [sg.Text('Selekcja'), sg.Combo(list(RealRepresentationParameters.signalMapping()['SELECTION'].keys()),default_value=SELECTION_map[GC.config.selection], key='_SELECTION_', enable_events=True, size=(25, 1))],
                    [sg.Text('Procent elity'), sg.Input(key='_ELITE_PERCENT_', enable_events=True, default_text=str(GC.config.elitePercent), size=(20,1))],
                ...
                sg.Column([
                    ...
                    [sg.Text('Rodzaj krzyżowania'),sg.Combo(list(RealRepresentationParameters.signalMapping()['CK'].keys()), default_value=CK_map[GC.config.chConfig.ck], key='_CK_',enable_events=True, size=(20, 1))],
                    [sg.Text('Prawdopodobienstwo krzyżowania'),sg.Input(key='_CP_', enable_events=True, default_text=str(GC.config.chConfig.cp), size=(20, 1))],

                    [sg.Text('Rodzaj mutacji'),sg.Combo(list(RealRepresentationParameters.signalMapping()['MK'].keys()), default_value=MK_map[GC.config.chConfig.mk], key='_MK_',enable_events=True, size=(20, 1))],
                    [sg.Text('Prawdopodobienstwo mutacji'),sg.Input(key='_MP_', enable_events=True, default_text=str(GC.config.chConfig.mp), size=(20, 1))],
                ]),
            ],
        ]
        self.setDefinition(definition)
